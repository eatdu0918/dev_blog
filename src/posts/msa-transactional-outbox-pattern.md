...
이러한 **Dual Write** 문제를 해결하기 위해, 우리는 DB와 메시지 브로커를 묶는 대신 **DB 트랜잭션의 원자성(Atomicity)**을 활용하기로 했습니다.
...
물론 Polling 방식은 DB 부하를 줄 수 있으므로, 트래픽이 매우 큰 경우 **Debezium**을 활용한 **CDC(Change Data Capture)** 방식으로의 전환도 고려해 볼 수 있습니다. 현재 우리 프로젝트 수준에서는 Polling 방식으로도 충분히 안정적인 정합성을 유지하고 있습니다.
