...
class BaseHttpClient {
    public void get() { System.out.println("API 호출 성공: 200 OK"); }
    public void get(int timeout) { System.out.println("타임아웃 " + timeout + "ms 설정 후 호출"); } // 오버로딩 허용
}
...
```javascript
class BaseHttpClient {
  get(timeout) { console.log(`타임아웃 ${timeout || 5} 옵션 인자 가변성 처리`); }
}

class CustomHttpClient extends BaseHttpClient {
  // 인자 갯수가 다르든 말든 냅다 덮어쓰고 오버라이드 파괴 선언
  get(timeout, useCache) {
    if (useCache) console.log("캐시 데이터 활용");
    else super.get(timeout);
  }
}
