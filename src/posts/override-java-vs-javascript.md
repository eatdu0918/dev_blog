...
class Animal {
    ...
    public void makeSound(int volume) { System.out.println("볼륨 " + volume); } // 오버로딩 허용
}

class Dog extends Animal {
    @Override
    public void makeSound() { System.out.println("멍멍"); } // 정상 오버라이딩
    
    // @Override public void makeSound(String type) -> 바로 빨간 줄 에러 폭격
}
```
...
```javascript
class Animal {
  makeSound(volume) { console.log(`볼륨 ${volume || 5} 웁니다.`); }
}

class Cat extends Animal {
  // 인자 갯수가 다르든 말든 냅다 덮어쓰고 오버라이드 파괴 선언
  makeSound(volume, isAngry) {
    if (isAngry) console.log("애옹 하악질");
    else super.makeSound(volume);
  }
}

// Java개발자 시점에선 경악스러운 자유도 허용
const myCat = new Cat();
myCat.makeSound(10, true);  
myCat.makeSound(); // 컴파일 패스, 런타임에 undefined 넣고 굴러감
```
